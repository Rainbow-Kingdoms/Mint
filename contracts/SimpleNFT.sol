...
contract SimpleNFT is ERC721A, Ownable {
    ...
    function seedDiscountList(address[] memory addresses)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++) {
            discountList[addresses[i]] = 1;
        }
    }

    function removeDiscountList(address[] memory addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            discountList[addresses[i]] = 0;
        }
    }
